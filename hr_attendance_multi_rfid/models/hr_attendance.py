...
class HrAttendanceExportWizard(models.TransientModel):
    ...
    def _get_default_attendances(self):
        return self.env['hr.attendance']

    from_date = fields.Datetime('From Date', default=lambda self: self._set_default_from())
    to_date = fields.Datetime('To Date', default=lambda self: self._set_default_to())
    export_format = fields.Selection(selection=[('acc2omz', 'Omeks2000 ACC2OMZ'), ('enersys', 'Omeks2000 Enersys')], default='acc2omz', required=True)
    state = fields.Selection([('choose', 'choose'), ('get', 'get')], default='choose')
    data = fields.Binary('File', readonly=True)
    name = fields.Char('FileName')
    no_content = fields.Char('No content', help='Displays a message if there is no content.')
    period = fields.Selection([('previous_month', 'Previous month'), ('custom_date', 'Custom date')], default='previous_month', required=True)
    ...
    def export_hr_attendances(self):
        ...
        def export_acc2omz(attendances, separator):
            ...
            for attendance in attendances:
                check_in = separator.join(["{}1".format(attendance.id), str(attendance.check_in.day), str(attendance.check_in.month), 
                                            str(attendance.check_in.year), str(attendance.check_in.hour), str(attendance.check_in.minute),
                                            str(attendance.check_in.second), str(attendance.hr_rfid_user_event_check_in_id.employee_id.barcode),
        ...
        def export_enersys(attendances, separator):
            ...
            for attendance in attendances:
                ...
                check_in = separator.join(["М: 001", "ENTERANCE", 
        ...
        attendances = this.env['hr.attendance'].search([('create_date', '>=', from_date), ('create_date', '<=', to_date)])
        ...
        if not filecontent:
            this.write({'no_content': 'There are not any attendances for the given period {} - {}'
                        .format(datetime.strftime(from_date, '%d.%m.%y'), datetime.strftime(to_date, '%d.%m.%y'))})
