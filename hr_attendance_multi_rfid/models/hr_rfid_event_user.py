    ...
    def create(self, vals_list):
        def check_check_in(cev):
            if cev.employee_id.attendance_state == "checked_out":
                cev.in_or_out = "0"
                cev.employee_id.attendance_action_change_with_date(
                    cev.event_time, cev.id
                )
        ...
        for vals in vals_list:
            ...
            if ev.door_id.attendance is True and ev.event_action == "1":
                if ev.reader_id.mode == "03":  # Workcode mode
                    ...
                    ev.reader_id.reader_type == "0"
                    and ev.employee_id.attendance_state == "checked_out"
                ):
                    ev.in_or_out = "0"
                    ev.employee_id.attendance_action_change_with_date(
                        ev.event_time, ev.id
                    )
                elif (
                    ev.reader_id.reader_type == "1"
                    and ev.employee_id.attendance_state == "checked_in"
                ):
                    ev.in_or_out = "1"
                    ev.employee_id.attendance_action_change_with_date(
                        ev.event_time, ev.id
