...
Contains technical details about the hardware communication."""
    ...
    def _gc_delete_old_vending_events(self):
        ...
        Note: Odoo 17 autovacuum ignores the return value (no re-queue), so the
        per-company max batch limit caps work per run; remaining records are
        removed on the next scheduled autovacuum pass.

        Returns:
            tuple: (total_deleted, has_more)
        """
        batch_size = 1000
