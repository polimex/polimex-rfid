...
class ProjectCustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'barcode_count' in counters:
            values['barcode_count'] = request.env['hr.rfid.card'].search_count([('card_type', '=', self.env.ref('hr_rfid.hr_rfid_card_type_barcode'))]) \
    ...
    @http.route('/webcard/<int:card_id>', type='http', auth='public', website=True)
    def portal_my_project_task(self, card_id=None, access_token=None, **kw):
        try:
            project_sudo = self._document_check_access('hr.rfid.card', card_id, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        Card = request.env['hr.rfid.card']
        if access_token:
            Card = Card.sudo()
        card_sudo = Card.search([('id', '=', card_id)], limit=1).sudo()
        # task_sudo.attachment_ids.generate_access_token()
        values = self._task_get_page_view_values(card_sudo, access_token, **kw)
