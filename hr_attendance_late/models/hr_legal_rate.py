...
class HrLegalRate(models.Model):
    ...
    def init(self):
        # Two partial UNIQUE indexes instead of one UNIQUE(code, date_from,
        # company_id): PostgreSQL treats NULL as distinct in a UNIQUE
        # constraint, so a plain 3-column unique would NOT stop duplicate
        # GLOBAL rates (company_id IS NULL) - and two effective global rows
        # for the same code/date make the lookup non-deterministic. Split by
        # company-set vs global.
        #
        # Odoo 17 has no ``models.UniqueIndex`` declarative helper (v19-only)
        # and ``_sql_constraints`` cannot express a partial (WHERE) index, so
        # the canonical core approach is raw ``CREATE UNIQUE INDEX`` in
        # ``init`` - see product_product.py:194.
        self.env.cr.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS hr_legal_rate_unique_company_rate "
            "ON %s (code, date_from, company_id) WHERE company_id IS NOT NULL"
            % self._table
        )
        self.env.cr.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS hr_legal_rate_unique_global_rate "
            "ON %s (code, date_from) WHERE company_id IS NULL"
            % self._table
        )
