...
class RFIDNotification(models.Model):
    ...
    def make_message(self, event, partner_id):
        ...
        door_link = door_link and '<a href="%s">%s</a>' % (door_link, event.door_id.name) or None
        # generate controller link
        controller_link = controller_link and '<a href="%s">%s</a>' % (controller_link, event.controller_id.name) or None
        # generate webstack link
        webstack_link = webstack_link and '<a href="%s">%s</a>' % (webstack_link, event.webstack_id.name) or None
        # generate html code for partner link
        partner_link = partner_link and '<a href="%s">%s</a>' % (partner_link, partner_name) or None
        # generate html code for event link
        event_link = event_link and '<a href="%s">%s</a>' % (
    ...
    def notify_by_discuss(self, recipients, msg):
        ...
        for recipient in recipients:
            # Check for existing private channel
            channel = self.env['mail.channel'].search([
                ('channel_type', '=', 'chat'),
                ('public', '=', 'private'),
                ('channel_partner_ids', 'in', [odoobot_id, recipient.id]),
            ], limit=1)

            if not channel:
                # Create a new private channel
                channel = self.env['mail.channel'].create({
                    'channel_partner_ids': [(4, odoobot_id), (4, recipient.id)],
                    'public': 'private',
                    'channel_type': 'chat',
                })
