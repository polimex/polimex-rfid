...
class TestSotDenied(HttpCase):
    ...
    def setUpClass(cls):
        ...
        # Prefer an existing iCON115 (hw_version='11') with door+reader.
        # If none exists (e.g. fresh dev DB), build minimal hardware so the
        # test is self-contained.
        Ctrl = cls.env['hr.rfid.ctrl']
        Door = cls.env['hr.rfid.door']
        Reader = cls.env['hr.rfid.reader']
        cls.ctrl = False
        for c in Ctrl.search([('hw_version', '=', '11')]):
            if Door.search([('controller_id', '=', c.id)], limit=1) \
               and Reader.search([('controller_id', '=', c.id)], limit=1):
                cls.ctrl = c
                break
        if not cls.ctrl:
            cls._build_minimal_hardware()
        ...
        if not cls.real_card:
            contact = cls.env['res.partner'].create({
                'name': 'SOT Test Contact',
                'company_id': cls.webstack.company_id.id,
            })
            cls.real_card = cls.env['hr.rfid.card'].create({
                'number': '1000000001',
                'card_input_type': 'w34',
    ...
    @classmethod
    def _build_minimal_hardware(cls):
        """Create a webstack + iCON115 controller + door + reader when the
        test database doesn't already have such hardware."""
        ws = cls.env['hr.rfid.webstack'].create({
            'name': 'SOT-Test-WS',
            'serial': '1000001',
            'key': '0000',
            'active': True,
        ...
        # Some hr_rfid.ctrl flows auto-create doors/readers via F0 handshake;
        # build them explicitly here so the test does not need the handshake.
        if not cls.env['hr.rfid.reader'].search(
                [('controller_id', '=', ctrl.id)], limit=1):
            cls.env['hr.rfid.reader'].create({
                'name': 'R1',
                'number': 1,
                'reader_type': '0',
                'controller_id': ctrl.id,
            })
        if not cls.env['hr.rfid.door'].search(
                [('controller_id', '=', ctrl.id)], limit=1):
            reader = cls.env['hr.rfid.reader'].search(
                [('controller_id', '=', ctrl.id)], limit=1)
            ...
            if 'door_id' in reader._fields:
                reader.door_id = door.id
        cls.ctrl = ctrl
